...
export default function FeaturedProductsPage() {
  ...
  const [unfeaturing, setUnfeaturing] = useState<Record<string, boolean>>({});
  ...
  // ── Unfeature ──────────────────────────────────────────────────────────────
  async function handleUnfeature(product: Product) {
    setUnfeaturing((prev) => ({ ...prev, [product._id]: true }));
    // Optimistic remove
    setProducts((prev) => prev.filter((p) => p._id !== product._id));
    try {
      await api.patch(`/v2/products/${product._id}/featured`);
      success(`"${product.name}" unfeatured`);
    } catch (err: any) {
      // Revert
      setProducts((prev) => {
      ...
      toastError(err?.response?.data?.error ?? "Failed to unfeature product");
    } finally {
      setUnfeaturing((prev) => ({ ...prev, [product._id]: false }));
    }
  }
    ...
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="bg-slate-900/50 border border-slate-800 rounded-lg flex flex-col items-center justify-center py-20 text-slate-400 gap-3">
          ...
          <p className="text-sm font-medium text-slate-300">No featured products yet.</p>
          <p className="text-xs text-slate-500">
            Feature products from the Product Manager.
          </p>
              ...
              >
                ...
                <div className="p-4 flex flex-col gap-3 flex-1">
                  ...
                  <Button
                    ...
                    disabled={unfeaturing[product._id]}
                    variant="outline"
                    size="sm"
                    className="w-full border-orange-700/40 text-orange-400 hover:bg-orange-500/10 hover:text-orange-300 text-xs flex items-center justify-center gap-1.5"
                  >
                    {unfeaturing[product._id] ? (
